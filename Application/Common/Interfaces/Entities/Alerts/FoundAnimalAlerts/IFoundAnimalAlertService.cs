...
{
	Task<FoundAnimalAlertResponseWithGeoLocation> GetByIdAsync(Guid alertId);

	Task<PaginatedEntity<FoundAnimalAlertResponse>> ListFoundAnimalAlerts(
		FoundAnimalAlertFilters filters, int page, int pageSize);

	Task<FoundAnimalAlertResponse> CreateAsync(CreateFoundAnimalAlertRequest request, Guid userId);
